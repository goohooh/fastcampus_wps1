...
class LinkedList :
    ...
    def add_between(self, element, between_first_num):
        node = self._Node(element)
        check_search = self._head

        while(check_search._element != between_first_num):
            check_search = check_search._next#while문을 통해서 비트윈 값과 일치하는 노드 전의 노드를
            #check_search로 선언해버림

        check_search = check_search._next#하지만 우리가 필요한건 비트윈 값과 정확히 일치하는 노드이기
        #때문에 check_search의 다음 노드를 check_search로 넣어줌

        node._next = check_search._next#추가하려고 만들어준 노드(node)의 next는 check_search함수의 next값이다.

        check_search._next = node
        node._next._prev = node#생성된 노드의 다음 값의 이전 값은 생성된 노드이다.(헛소리 같지만 그림 그려보면 이해됨)
        node._prev = check_search
